...
<body>
	...
	<div id="container">
		...
		<div id="body">
			...
			<table>
			<tr><th>Título</th><th>Duración</th><th>Artista</th></tr>
			<?php foreach($consulta->result() as $canciones):?>
			<tr><td><center><?php echo $canciones->titulo ?></center></td><td><center><?php echo $canciones->duracion ?></center></td><td id='artistas'  style='cursor:pointer' url="<?php echo base_url();?>artistas/index.php" artista='<?php echo $canciones->artistas?>'><center><?php echo $canciones->artistas ?></center></td></tr>
			<?php endforeach;?>
			</table>
		</div>
	...
	<script>
		$("#artistas").on("click",function(){
			$.ajax({
				url:$(this).attr("url"),
				type:'POST',
				data:{artista:$(this).attr("artista")},
